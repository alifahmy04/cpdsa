...
public:
    ...
    int verifyFrequencies(node* current, int l, int r) {
        if (r == -1) {
            return 0;
        }
        if (current->low == current->high) {
            return r - l + 1 == nums[current->elements[0]];
        }
        int leftL = count(current, l - 1, true); int leftR = count(current, r, true) - 1;
        int rightL = count(current, l - 1, false); int rightR = count(current, r, false) - 1;
        int leftResult = verifyFrequencies(current->left, leftL, leftR);
        int rightResult = verifyFrequencies(current->right, rightL, rightR);
        return leftResult + rightResult;
    }
    
    int verifyFrequencies(int l, int r) {
        return verifyFrequencies(root, l, r);
    }
};
...
int main() {
    ...
    int rank = 0;
    for (int i = 0; i < n; i++) {
        if (compression.find(mapping[i]) == compression.end()) {
            compression[mapping[i]] = rank;
            reverse[rank] = mapping[i];
            rank++;
        }
    }
    for (int i = 0; i < n; i++) {
        nums[i] = compression[nums[i]];
    }
    wavelet_tree wt(nums);
    for (int i = 0; i < m; i++) {
        int l, r, k;
        cin >> l >> r >> k; 
        cout << reverse[wt.kthMinimum(k + 1, l, r - 1)] << nl;
    }
}