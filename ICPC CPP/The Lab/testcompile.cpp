...
public:
    ...
    void extend(char c) {
        if (states[last].next.find(c) != states[last].next.end() && states[states[last].next[c]].len == states[last].len + 1) {
    ...
    int countSubstring(const string& s) {
        ...
        for (int i = 0; i < s.size(); i++) {
            if (states[cur].next.find(s[i]) == states[cur].next.end()) {
                return false;
            }
    ...
    string lcs(string& s, int n) {
        ...
        for (int i = 0; i < s.size(); i++) {
            while (v && !(states[v].next.find(s[i]) != states[v].next.end() && states[states[v].next[s[i]]].count >= n)) {
                v = states[v].link;
                l = states[v].len;
            }
            if (states[v].next.find(s[i]) != states[v].next.end() && states[states[v].next[s[i]]].count >= n) {
                v = states[v].next[s[i]];
                l++;
            }
...
signed main() {
    ...
    int max_length = 0;
    for (state& s : sa.states) {
        if (s.count == count) {
            max_length = max(max_length, s.len);
        }
    }
    cout << max_length << nl;
}
